...
use ff::PrimeField;
use nova_scotia::{calculate_witness, r1cs::CircomConfig, synthesize, generate_witness_from_wasm};
...
pub fn sha256_circom<F: PrimeField, CS: ConstraintSystem<F>>(
    ...
    let mut witness_dir = root.clone();
    witness_dir.push("main_js");
    let mut witness_output = root.clone();
    witness_output.push("output.wtns");
    let witness_input_json = format!("{{ \"arg_in\": [\"{}\", \"{}\"] }}", 0, 0);
    let other_witness = generate_witness_from_wasm(witness_dir, witness_input_json, witness_output);

    assert_eq!(witness, other_witness);
